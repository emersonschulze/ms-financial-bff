...
export async function PATCH(
  ...
  logger.info('[type-expenses] PATCH inactivate request received', { id });

  try {
    const updated = await service.inactivate(Number(id));
    const adapted = adaptTypeExpense(updated);

    logger.info('[type-expenses] PATCH inactivate completed', { id });

    return NextResponse.json(adapted);
  } catch (error) {
    return handleError(error, '[type-expenses] PATCH inactivate failed', id);
  }
}
